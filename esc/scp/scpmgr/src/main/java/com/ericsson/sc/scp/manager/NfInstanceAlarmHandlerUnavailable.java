...
public class NfInstanceAlarmHandlerUnavailable
{
    ...
    static final String NF_INSTANCE_UNAVAILABLE_ALARM = "ScpNfInstanceUnavailable";
    static final String NF_INSTANCE_UNAVAILABLE_DESC = "SCP, NF Instance Unavailable";
    ...
    private static class FaultyNfInstance
    {
        ...
        public String constructAdditionalInfo(DataOfNfInstance dataPerNf)
        {
            ...
            additionalInfo.append("nf=scp-function,nf-instance=").append(dataPerNf.getNfInst()).append(",nf-pool=").append(dataPerNf.getPoolName()).append(",");
    ...
    private static class DataOfNfInstance
    {
        ...
        private Double valueOf2xx; // number of 5xx failures
        ...
        private Double lastAlarm; // the sum of valueOf5xx and valueOfTm when the alarm ceased
                                             ...
                                             BehaviorSubject<Optional<EricssonScp>> configFlow)
    {
        ...
        var class2xxchanges = Query.metric("scp_egress_pool_nf_class2_sum");
        var class5xxchanges = Query.metric("scp_egress_pool_nf_class5_sum");
        var classTimeoutChanges = Query.metric("scp_egress_pool_nf_timeout_sum");
        var classPendingFailureEjectChanges = Query.metric("scp_egress_pool_nf_pending_failure_eject_sum");
     ...
     * envoy_upstream_rq_xx_per_nf counter with 2xx,5xx envoy status code and from
     * envoy_upstream_rq_timeout_per_nf counter have been fetched
     * 
     * @param dataUpdated
     * 
     * 
     ...
     * @param scpCfg
     */
    private synchronized void configChanges(Optional<EricssonScp> scpCfg)
    {
        log.debug("Configuration update received.");
        this.scpCfg = scpCfg;
        if (this.scpCfg.isPresent())
        {
            this.nfData = CommonConfigUtils.getDataPerNf(this.scpCfg.get().getEricssonScpScpFunction());
            ...
            else
            {
                this.dataPerNfList.stream().filter(Objects::nonNull).forEach(nf ->
                {
                    ...
                    {
                        log.debug("The NF with id {}, has not enabled the ScpNfInstanceUnavailable alarm.", nf.getNfInstanceId());
                        try
                        {
                            handleAvailableStatus(nf);
                            nf.setRaised(false);
    ...
    /**
     * /** This method is used to create a new DataOfNfInstance object for each NF.
     * The param 'dataPerNf' contains all the information that fetched from PM. The
                                                  ...
                                                  String counter)
    {
        ...
        // nfData contains the address,nfType and the threshold per NF
