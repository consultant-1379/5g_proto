...
{
    static final String NF_SERVICE_INSTANCE_UNHEALTHY_ALARM = "SeppNfServiceInstanceActiveHealthCheckFailed";
    static final String NF_SERVICE_INSTANCE_UNHEALTHY_ALARM_DESC = "SEPP, The NF Service Instance active health check failed";

    static final String NF_NODE_UNREACHABLE_ALARM = "SeppNfServiceInstanceNotReachable";
    static final String NF_NODE_UNREACHABLE_DESC = "SEPP, The NF Service Instance is not reachable for service requests";
    ...
    private static class FaultyNfInstance
    {
        ...
        /**
         * Construct a NF Instance (static or discovered) for that an alarm "SEPP, NF
        ...
        public String addInfoToJson()
        {
            ...
            fullRdn.append("nf=sepp-function,nf-instance=")
    ...
    private void startEventHandler()
    {
        ...
        configFlow.retry().subscribe(cfg ->
        {
            ...
            {
                if (seppCfg.get().getEricssonSeppSeppFunction().getNfInstance().size() == 0)
                {
                    log.info("Configuration is Empty/Deleted");
                    for (FaultyNfInstance faultyNfInst : faultyNfInstances.values())
                    {
                ...
                {
                    log.info("Configuration is presented");

                    for (FaultyNfInstance faultyNfInst : faultyNfInstances.values())
                    {
                        if (!CommonConfigUtils.isNfServiceNameValid(this.seppCfg.get().getEricssonSeppSeppFunction(), faultyNfInst.getAdditionalInfo()))
                        {
                            alarmNfNodeNotReachableCease(faultyNfInst);
                            faultyNfInstances.remove(faultyNfInst.getAlarmRdn());
                        }
    ...
    private void handleUnreachableStatus(EnvoyStatus hostStatus) throws JsonProcessingException
    {
        ...
        if (this.seppCfg.isPresent())
        {
            List<Map<String, String>> nfInstanceRdns = CommonConfigUtils.getNfInstanceRdn(this.seppCfg.get().getEricssonSeppSeppFunction(),
                                                                                          hostStatus.getUrl(),
            ...
            int blockingTime = CommonConfigUtils.getTempBlockingTime(this.seppCfg.get().getEricssonSeppSeppFunction(), hostStatus.getCluster());
    ...
    private void alarmNfNodeNotReachableRaise(FaultyNfInstance faultyNfInstance) throws JsonProcessingException
    {
        ...
        this.alarmService.raise(faultIndication)
                         .subscribe(() -> log.info("raised alarm {}", NF_NODE_UNREACHABLE_ALARM),
    ...
    private void alarmNfNodeNotReachableCease(FaultyNfInstance faultyNfInstance) throws JsonProcessingException
    {
        var faultIndication = new FaultIndicationBuilder().withFaultName(NF_NODE_UNREACHABLE_ALARM)
                                                          ...
                                                          .build();
        this.alarmService.cease(faultIndication)
                         .subscribe(() -> log.info("ceased alarm {}", NF_NODE_UNREACHABLE_ALARM),
                                    t -> log.error("Error raising alarm. Cause: {}", t.toString()));
    }
}
