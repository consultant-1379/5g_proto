...
- name: bsf_vertx
  ...
  - record: bsf_http_server_response_time_seconds
    expr: sum by (nf,nf_instance,method) (rate(vertx_http_server_response_time_seconds_sum{nf="bsf",route=~'/nbsf-management/v1.*'}[45s]))/ sum by (nf,nf_instance,method)  (rate(vertx_http_server_response_time_seconds_count{nf="bsf",route=~"/nbsf-management/v1.*"}[45s])) >=0
    labels:
      service: "nbsf-management"

  - record: bsf_http_server_response_time_seconds_max
    expr:  max by (nf,nf_instance,method) (vertx_http_server_response_time_seconds_max{nf="bsf",route=~"/nbsf-management/v1.*"})
    labels:
      service: "nbsf-management"
...
- name: bsf_cpu_usage
  ...
  - record: job:wcdb_avg_cpu_load_millicores
    expr: avg(rate(container_cpu_usage_seconds_total{container='cassandra',namespace='{{ .Release.Namespace }}'}[3m]))*1000 or vector(0)
...
- name: bsf_memory_usage
  ...
  - record: job:wcdb_avg_memory_mb
    expr: avg(container_memory_working_set_bytes{container='cassandra',namespace='{{ .Release.Namespace }}'})/1024/1024 or vector(0)
...
- name: cassandra_metrics
  ...
  - record: job:file_system_storage_usage
    expr: sum(cassandra_storage_filesystem_bytes_total - cassandra_storage_filesystem_usable_bytes) by (instance)
  ...
  - record: job:cassandra_node_unavailable
    expr: (sum((cassandra_endpoint_downtime_seconds_total{endpoint_datacenter='{{ index .Values "eric-bsf" "cassandra" "datacenter" }}'} != bool 0)) by (endpoint)) >= bool ({{ index .Values "eric-data-wide-column-database-cd" "replicaCount" | int}} - 1)

  # Number of cassandra node interconnections between datacenters
  - record: job:cassandra_datacenter_unavailable
    expr: (sum(cassandra_endpoint_active{endpoint_datacenter!='{{ index .Values "eric-bsf" "cassandra" "datacenter"  }}'}) by (endpoint_datacenter)) <= bool 0
 
  # Number of Estimated Partitions per Table
  - record: cassandra_table_estimated_partitions_by_table
    expr: cassandra_table_estimated_partitions{keyspace='nbsf_management_keyspace'} >= 0
