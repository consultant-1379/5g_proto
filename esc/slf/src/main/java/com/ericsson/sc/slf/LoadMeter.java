...
public class LoadMeter implements IfCountProvider
{
    ...
    private static final Gauge gcSlfLoad = MetricRegister.singleton()
                     ...
                     final Observable<Optional<EricssonScp>> config)
    {
        ...
        // In regular intervals, fetch from Prometheus the average load of SLF.

        final Query.Element cpuUsage100Avg = Query.avg(Query.metric("job:container_cpu_usage_100").param("container", "eric-sc-slf"));
        ...
        this.config = config.toFlowable(BackpressureStrategy.LATEST)//
                            ...
                            .doOnNext(o ->
                            {
                                final var scpFunction = o.get().getEricssonScpScpFunction();
                                ...
                                if (scpFunction != null && !scpFunction.getNfInstance().isEmpty())
                                {
                                    ...
                                    if (nfInstance != null)
                                    {
                                        this.labelValues = MetricRegister.rdnToLabelValues(new Rdn("nf", "scp-function").add("nf-instance",
                                                                                                                             nfInstance.getName()))
    ...
    private synchronized void setCount(List<PmAdapter.Query.Response.Data> data)
    {
        ...
        if (this.labelValues != null)
        {
            ...
            gcSlfLoad.labels(this.labelValues).set(this.totalLoad);
        }
    }
}
