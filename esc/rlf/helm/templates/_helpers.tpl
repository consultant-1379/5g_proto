...
*/}}
{{- define "eric-sc-rlf.fileCollector.enabled" -}}
{{- if  .Values.global -}}
...
{{- if .Values.global -}}
    {{- if .Values.global.ericsson -}}
        {{- if .Values.global.ericsson.scp -}}
            {{- if hasKey .Values.global.ericsson.scp "enabled" -}}
                {{- if .Values.global.ericsson.scp.enabled -}}
                    {{ $nfname = "SCP" }}
                {{- end -}}
            {{- end -}}
        {{- end -}}
...
{{- if .Values.global -}}
    {{- if .Values.global.ericsson -}}
        {{- if .Values.global.ericsson.sepp -}}
            {{- if hasKey .Values.global.ericsson.sepp "enabled" -}}
                {{- if .Values.global.ericsson.sepp.enabled -}}
                    {{- if eq $nfname "" -}}
                        {{ $nfname = "SEPP" }}
                    {{- else -}}
                        {{ $nfname = "SCP,SEPP" }}
    ...
    {{- end -}}
{{- end -}}
{{ printf "ericsson.com/nf-name: \"%s\"" $nfname }}
{{- end}}
