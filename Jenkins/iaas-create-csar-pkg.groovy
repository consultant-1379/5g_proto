        ...
        stage('Fail for parallel running creation')
        {
            steps
            {
                script
                {
                    ...
                    {
                        PARALLEL_CREATION = sh(script: """curl -u $API_TOKEN  https://fem1s10-eiffel029.eiffel.gic.ericsson.se:8443/jenkins/job/5G-ESC/job/CSAR/job/csar-from-spider/lastBuild/api/json | jq -r . | grep inProgress """, returnStdout: true).trim()
                    }
                    if ("${PARALLEL_CREATION}".contains("true"))
                    {
                        if ("${env.MAIL}" != 'null')
                        {
                            def emailbody_job_failure_1="\nthere is another csar package creation ongoing by the job \"csar-from-spider\" (https://fem1s10-eiffel029.eiffel.gic.ericsson.se:8443/jenkins/job/5G-ESC/job/CSAR/job/csar-from-spider/lastBuild/consoleFull)"
                        ...
                        println("There is another csar package creation ongoing by the job \"csar-from-spider\" (https://fem1s10-eiffel029.eiffel.gic.ericsson.se:8443/jenkins/job/5G-ESC/job/CSAR/job/csar-from-spider/lastBuild/consoleFull)")
                    ...
                    {
                        println ("There is no other csar package creation ongoing. This process continues.")
                    }
                }
            }
        }
...
//              Check space for images
        ...
        stage('Archiving & Mail notification csar pkg creation')
        {
            steps
            {
                ...
                sh """echo "APPLICATION=eric-sc"                         > ./artifact.properties"""
                ...
                script
                {
                    if ("${env.MAIL}" != 'null')
                    {
                        def emailbody_csar_created_1="\na new csar package called eric-sc-${CSAR_PACKAGE_VERSION}.unsigned.csar has been created from"
