...
pipeline
{
    ...
    {
        IAAS_NODE = "${IAAS_NODE}"
        SFTP_SERVER_NAMESPACE = "${SFTP_SERVER_NAMESPACE}"
...
//                sh "sudo rm -rf ../${JOB_BASE_NAME}_${IAAS_NODE}@2*"
                ...
                script
                {
                    ...
                    // Check that mandatory common parameters contain a value

                    if ( ("${env.SFTP_SERVER_NAMESPACE}" == 'null') || ("${env.CSAR_PKG_FOLDER}" == 'null') )
                    {
                        error('Variables SFTP_SERVER_NAMESPACE and CSAR_PKG_FOLDER must contain a value. Execution stopped!')
                    }
                    ...
                    DAFT_NS_DIR = "/home/eccd/daft_ns_${NAMESPACE}"
                } //script
            }
        }
        ...
        stage('Trigger Disaster Recovery')
        {
            steps
            {
                sh "${DAFT_DIR}/expect/bin/send_command_to_ssh.exp --timeout=3600  --user=eccd --ip=${NODE_DIRECTOR_IP} --password='notneeded' \
                --stop-on-error --command='perl ${DAFT_NS_DIR}/perl/bin/execute_playlist.pl -p 402_Maintainability_Test_Disaster_Recovery \
                -n ${DAFT_NS_DIR}/${NW_CONFIG_FILE_NAME} ${NW_CONFIG_OPTION} ${PLAYLIST_PARAMETERS} -v SFTP_NAMESPACE=${SFTP_SERVER_NAMESPACE} \
                -j ${IAAS_NODE}_${JOB_BASE_NAME}-${BUILD_NUMBER} -v CREATE_PACKED_WORKSPACE_FILE_AT_FAILURE=yes \
                -v CREATE_PACKED_WORKSPACE_FILE_AT_SUCCESS=yes -v PACK_TROUBLESHOOTING_LOGS_SEPARATELY=yes -v INPUT_AVAILABLE=no \
                -c no -w ${DAFT_LOGS_DIR}'"
            }
            post
            {
                always
                {
                    script
                    {
                        LOGS_FOLDER_DISASTER_RECOVERY = """${sh( returnStdout: true,
                ...
                failure
                {
                    script
                    {
                        if ("${env.MAIL}" != 'null')
                        {
                            def emailbody_tc_failure_1="\nthe disaster recovery via DAFT on node ${IAAS_NODE} has failed.\n\nDetailed logs regarding the test case can be found"
                            def emailbody_tc_failure_2=" on the director of node ${IAAS_NODE} (IP address of director: ${NODE_DIRECTOR_IP}) under the folder: ${LOGS_FOLDER_DISASTER_RECOVERY}"
                            emailext body: "${emailbody_start}"+"${emailbody_tc_failure_1}"+"${emailbody_tc_failure_2}"+"${emailbody_end}",
                            subject: "Automatic IaaS execution of disaster recovery failed", to: "${MAIL}"
                        }
                    }
                }
            }
        ...
        stage('Notification mail')
        {
            steps
            {
                script
                {
                    if ("${env.MAIL}" != 'null')
                    {
                        def emailbody_job_finished_1="\nthe automatic disaster recovery via DAFT on node ${IAAS_NODE} has concluded.\n\nDetailed logs regarding the test case can be found on the"
                        def emailbody_job_finished_2=" director of node ${IAAS_NODE} (IP address of director: ${NODE_DIRECTOR_IP}) under the folder: ${LOGS_FOLDER_DISASTER_RECOVERY}"
                        def emailbody_job_finished_3="\nDetailed logs for the overall disaster recovery jenkins job can be found under: ${LOGS_LINK}/$BUILD_NUMBER/consoleFull"
                        emailext body: "${emailbody_start}"+"${emailbody_job_finished_1}"+"${emailbody_job_finished_2}"+
                        "${emailbody_job_finished_3}"+"${emailbody_end}",
                        subject: "Automatic IaaS execution of disaster recovery concluded", to: "${MAIL}"
                    }
                }
            }
        }
    ...
    post
    {
        ...
        failure
        {
            script
            {
                if ("${env.MAIL}" != 'null')
                {
                    def emailbody_job_failure_1="\nthe job for executing a disaster recovery TC via DAFT on the IaaS node ${IAAS_NODE} has failed.\n\n"
                    ...
                    subject: "Automatic IaaS job of disaster recovery failed", to: "${MAIL}"
                }
            }
        }
    }
