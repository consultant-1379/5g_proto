...
//                sh "sudo rm -rf ../${JOB_BASE_NAME}_${IAAS_NODE}@2*"
                ...
                script
                {
                    ...
                    DAFT_NS_DIR = "/home/eccd/daft_ns_${DAFT_NS}-backup_restore"
                }
            }
        }
        ...
        stage('Trigger backup & restore')
        {
            steps
            {
                script
                {
                    echo 'Trigger backup & restore'
                    sh "${DAFT_DIR}/expect/bin/send_command_to_ssh.exp --timeout=3600  --user=eccd --ip=${NODE_DIRECTOR_IP} --password='notneeded' \
                    --stop-on-error --command='perl  ${DAFT_NS_DIR}/perl/bin/execute_playlist.pl -p 401_Maintainability_Test_Backup_And_Restore -c no \
                    -n ${DAFT_NS_DIR}/${NW_CONFIG_FILE_NAME} ${NW_CONFIG_OPTION} -j ${IAAS_NODE}_${JOB_BASE_NAME}-${BUILD_NUMBER} -v SFTP_NAMESPACE=${SFTP_SERVER_NAMESPACE} \
                    -v CREATE_PACKED_WORKSPACE_FILE_AT_FAILURE=yes -v CREATE_PACKED_WORKSPACE_FILE_AT_SUCCESS=yes -v PACK_TROUBLESHOOTING_LOGS_SEPARATELY=yes -v INPUT_AVAILABLE=no -w ${DAFT_LOGS_DIR} '"
                } //script
            }
            post
            {
                always
                {
                    script
                    {
                        LOGS_FOLDER_BACKUP_RESTORE = """${sh( returnStdout: true,
                ...
                failure
                {
                    script
                    {
                        if ("${env.MAIL}" != 'null')
                        {
                            def emailbody_bupres_failure_1="\nthe automatic backup-restore via DAFT"
                            def emailbody_bupres_failure_2=" on node ${IAAS_NODE} has failed.\n\nDetailed logs can be found on the"
                            def emailbody_bupres_failure_3=" director of node ${IAAS_NODE} (IP address of director: ${NODE_DIRECTOR_IP}) under the folder: ${LOGS_FOLDER_BACKUP_RESTORE}"

                            emailext body: "${emailbody_start}"+"${emailbody_bupres_failure_1}"+"${emailbody_bupres_failure_2}"+"${emailbody_bupres_failure_3}"+"${emailbody_end}",
                            subject: "Automatic IaaS execution for backup restore (${APP}) failed", to: "${env.MAIL}"
                        }
                    }
                }
            }
        ...
        stage('Notification mail')
        {
            steps
            {
                script
                {
                    if ("${env.MAIL}" != 'null')
                    {
                        def emailbody_job_finished_1="\nthe automatic backup-restore via DAFT on node ${IAAS_NODE} has concluded."
                        def emailbody_job_finished_2="\n\nDetailed logs for the backup-restore itself can be found on the director of node ${IAAS_NODE} (IP address of director:"
                        def emailbody_job_finished_3=" ${NODE_DIRECTOR_IP}) under the folder: ${LOGS_FOLDER_BACKUP_RESTORE} \nDetailed logs for the overall backup-restore"
                        ...
                        subject: "Automatic IaaS backup-restore (${APP}) concluded", to: "${env.MAIL}"
                    }
                }
            }
        }
        ...
        failure
        {
            script
            {
                if ("${env.MAIL}" != 'null')
                {
                    ...
                    subject: "Automatic IaaS backup-restore (${APP}) failed", to: "${env.MAIL}"
                }
            }
        }
    }
