...
public final class DscLoad extends AbstractDefaultScenario implements DefaultScenario, Processor
{
    private GeneratedCert cert;
    private Processor nextProcessor;

    private static final String NAME = "dscload";

    private static final String SECRET_NAME = "dscload-certificates-secret";

    private static final String FQDN = "eric-dscload";

    /**
     * @param config
     */
    public DscLoad(Configuration config)
    {
        super(config);
    }
    ...
    public void installCerts(KubernetesClient kubeClient) throws DefaultScenarioException
    {
        try
        {
            var map = Map.of("key.pem", Utils.toPemString(this.cert.getPrivateKeyPrivateKey()), "cert.pem", Utils.toPemString(this.cert.getX509Certificate()));
