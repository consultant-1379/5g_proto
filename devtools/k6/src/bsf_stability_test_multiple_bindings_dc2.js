...
//in place constants, in case manual certificate and key are required
//const CERT = ``;
//const KEY = ``;

export let options = {
    duration: duration,
    setupTimeout: "18000s", // time available for setup function
    tlsAuth: [{
	    domains: [`${__ENV.BSF_WORKER_IP}`],
        cert: __ENV.CERT, //cert: CERT,
        key: __ENV.KEY, //key: KEY,
    }],
...
const bsf = `https://${__ENV.BSF_WORKER_IP}:${__ENV.BSF_WORKER_PORT}/`;
const numberOfBindings = `${__ENV.NUMBER_OF_BINDINGS}`;
const numberOfBindingsInt = parseInt(numberOfBindings); //convert String to Integer
...
export function setup() {
    make_post_requests_rangeA(numberOfBindingsInt);

    // Use 50% of rangeA for multiple bindings resolution counter
    var multipleBindings = Math.floor(numberOfBindingsInt/2);
    make_post_requests_rangeA(multipleBindings);
}
...
export default function() {
    make_get_requests(numberOfBindingsInt); // those that are posted within range A
    let bindingdIds = make_post_requests_rangeB(numberOfBindingsInt);
...
function make_post_requests_rangeA(n) {
    ...
    {
        let body = create_body(startValue+i);
...
function make_post_requests_rangeB(n) {
    ...
    for(let i = 0; i < n; i++)
    {
        let body = create_body(startValue + i + n); // [n, 2n)
        requests[i] = ["POST", postEndpoint, body, headers];
    }
...
function make_get_requests(n) {
    ...
    for(let i = 0; i < n; i++)
    {
        let ip = intToIP(i + startValue);
        requests[i] = ["GET", ipGetEndpoint + intToIP(startValue+i), null, headers];
    }
