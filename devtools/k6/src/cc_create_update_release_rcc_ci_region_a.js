...
export default function () {
    ...
    let ids = create_operation(smf_host, scp, headers, `${__ENV.CC_CREATE}`);
    if (ids.length > 0) {
        update_operation(ids, smf_host, scp, headers, `${__ENV.CC_UPDATE}`);
        notify_operation(scp, `${__ENV.CC_NOTIFY}`);
        release_operation(ids, smf_host, scp, headers, `${__ENV.CC_RELEASE}`);
    }

    if (ingressMSHeaders > 0) {
        send_ms_rcc_traffic(smf_host, scp, "MS_IngressHeaders", ingressMSHeaders);
    }

    if (egressMSHeaders > 0) {
        send_ms_rcc_traffic(smf_host, scp, "MS_EgressHeaders", egressMSHeaders);
    }

    if (ingressMSBody > 0) {
        send_ms_rcc_traffic(smf_host, scp, "MS_IngressBody", ingressMSBody);
    }

    if (egressMSBody > 0) {
        send_ms_rcc_traffic(smf_host, scp, "MS_EgressBody", egressMSBody);
    }

    if (combinedMS > 0) {
        send_ms_rcc_traffic(smf_host, scp, "MS_Combined", combinedMS);
    }
}

//Function to send traffic per message screening param
function send_ms_rcc_traffic(smf, scp, msHeader, traffic) {
    let headers_ms = headers;
    headers_ms["headers"]["ms-header"] = msHeader;
    headers_ms["headers"]["dummy-header-2"] = "";
    create_operation(smf, scp, headers_ms, traffic);
}

//Converged charging create operation
function create_operation(smf_host, scp, rcc_headers, traffic) {
    if (traffic > 0) {
        const method = "POST";
        ...
        for (let i = 0; i < traffic; i++) {
            requests[i] = [method, scp + uri, create_body, rcc_headers];
        }
        ...
        for (let i = 0; i < resp_length; i++) {
            check(responses[i], {
                "cc create (rcc region-a): protocol is HTTP/2": (r) => r.proto === 'HTTP/2.0',
                "cc create (rcc region-a): status was 201": (r) => r.status == 201
            });
        }
...
function update_operation(ids, smf_host, scp, rcc_headers, traffic) {
    if (traffic > 0) {
        const method = "POST";
        ...
        for (let i = 0; i < traffic; i++) {
            ...
            requests[i] = [method, scp + uri + ids[counter] + "/update", update_body, rcc_headers];
            counter++;
        }
        ...
        for (let i = 0; i < resp_length; i++) {
            check(responses[i], {
                "cc update (rcc region-a): protocol is HTTP/2": (r) => r.proto === 'HTTP/2.0',
                "cc update (rcc region-a): status was 200": (r) => r.status == 200
            });
        }
    }
}
...
function release_operation(ids, smf_host, scp, rcc_headers, traffic) {
    ...
    if (traffic > 0) {
        ...
        for (let i = 0; i < traffic; i++) {
            ...
            requests[i] = [method, scp + uri + ids[counter] + "/release", release_body, rcc_headers];
            counter++;
        }
        ...
        for (let i = 0; i < resp_length; i++) {
            check(responses[i], {
                "cc release (rcc region-a): protocol is HTTP/2": (r) => r.proto === 'HTTP/2.0',
                "cc release (rcc region-a): status was 204": (r) => r.status == 204
            });
        }
    }
}
...
function get_create_body(smf_host) {
    return JSON.stringify({
        "subscriberIdentifier": "imsi-460030100000001",
...
function notify_operation(scp, traffic) {
    ...
    if (traffic > 0) {
        ...
        for (let i = 0; i < resp_length; i++) {
            check(responses[i], {
                "cc notify (rcc region-a): protocol is HTTP/2": (r) => r.proto === 'HTTP/2.0',
                "cc notify (rcc region-a): status was 204": (r) => r.status == 204
            });
        }
    }
}
