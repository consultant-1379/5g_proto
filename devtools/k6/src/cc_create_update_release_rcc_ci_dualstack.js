...
export default function() {
  ...
  if(host_v4 && host_v4 !== "null") {
    ...
    let ids = create_operation(smf_host, scp_v4, `${__ENV.CC_CREATE_V4}`);
    if (ids.length > 0) {
      update_operation(ids, smf_host, scp_v4, `${__ENV.CC_UPDATE_V4}`);
      release_operation(ids, smf_host, scp_v4, `${__ENV.CC_RELEASE_V4}`);
    }
  }
  ...
  if(host_v6 && host_v6 !== "null") {
    ...
    let ids = create_operation(smf_host, scp_v6, `${__ENV.CC_CREATE_V6}`);
    if (ids.length > 0) {
      update_operation(ids, smf_host, scp_v6, `${__ENV.CC_UPDATE_V6}`);
      release_operation(ids, smf_host, scp_v6, `${__ENV.CC_RELEASE_V6}`);
    }
  }
}
...
function create_operation(smf_host, scp, reqNumber) {
  ...
  if(reqNumber>0){
    ...
    for(let i=0;i<reqNumber;i++) {
        let create_body = get_create_body(smf_host, i);
...
function update_operation(ids, smf_host, scp, reqNumber) {
  ...
  if(reqNumber>0){
    ...
    for(let i=0;i<reqNumber;i++) {
      if(counter>=ids.length){
        ...
        let update_body = get_update_body(smf_host, counter);
...
function release_operation(ids, smf_host, scp, reqNumber) {
...
function get_create_body(smf_host, i) { 

    var vu = `${__VU}`;
    var it = `${__ITER}`;
    var lvu = vu.length;
    var lit = it.length;
    var li  = i.toString().length;

    return JSON.stringify({
        "subscriberIdentifier": "imsi-46003" + "0".repeat(2-lvu) + `${vu}` + "0".repeat(6-lit) + `${it}` + "0".repeat(2-li) + `${i}`,
...
function get_update_body(smf_host, i) {
...
function get_release_body(smf_host, i) { 
