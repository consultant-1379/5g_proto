...
function create_operation(smf_host, scp) {
	...
	if (create > 0) {
		...
		for (let i = 0; i < resp_length; i++) {
			check(responses[i], {
				"cc create (rcc region-c): protocol is HTTP/2": (r) => r.proto === 'HTTP/2.0',
				"cc create (rcc region-c): status was 201": (r) => r.status == 201
			});
		}
...
function update_operation(ids, smf_host, scp) {
	...
	if (update > 0) {
		...
		for (let i = 0; i < resp_length; i++) {
			check(responses[i], {
				"cc update (rcc region-c): protocol is HTTP/2": (r) => r.proto === 'HTTP/2.0',
				"cc update (rcc region-c): status was 200": (r) => r.status == 200
			});
		}
	}
}
...
function release_operation(ids, smf_host, scp) {
	...
	if (release > 0) {
		...
		for (let i = 0; i < resp_length; i++) {
			check(responses[i], {
				"cc release (rcc region-c): protocol is HTTP/2": (r) => r.proto === 'HTTP/2.0',
				"cc release (rcc region-c): status was 204": (r) => r.status == 204
			});
		}
	}
}
...
function notify_operation(scp) {
	...
	if (notify > 0) {
		...
		for (let i = 0; i < resp_length; i++) {
			check(responses[i], {
				"cc notify (rcc region-c): protocol is HTTP/2": (r) => r.proto === 'HTTP/2.0',
				"cc notify (rcc region-c): status was 204": (r) => r.status == 204
			});
		}
	}//end if notify>0
}
