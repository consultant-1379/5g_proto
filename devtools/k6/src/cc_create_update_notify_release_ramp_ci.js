...
let create = parseInt(`${__ENV.CC_CREATE}`); 
let update = parseInt(`${__ENV.CC_UPDATE}`); 
let release = parseInt(`${__ENV.CC_RELEASE}`); 
let notify = parseInt(`${__ENV.CC_NOTIFY}`); 
let total = create+update+release+notify;
  
let iterations = Math.ceil(parseInt(`${__ENV.RPS}`)/ total);
let holdduration = `${__ENV.HOLDDURATION}`;
let rampduration = `${__ENV.RAMPDURATION}`;
let maxVirtualUsers = `${__ENV.MAX_VUS}`;
...
export let options = {
    scenarios: {
        load_test: {
            ...
            startRate: 1,    // we start at 1 iterations per second = 100 RPS 
            timeUnit: '1s', 
            stages: [
                { target: iterations, duration: rampduration  }, // go from 100 RPS to wanted RPS value
                { target: iterations, duration: holdduration }, // hold at set RPS
            ],
...
export default function() {
    ...
    let ids = create_operation(smf_host, supi, csa);
    if (ids.length > 0) {
    	update_operation(ids, smf_host, supi, csa);
        notify_operation(csa);
    	release_operation(ids, smf_host, supi, csa);
    }
}


//Converged charging create operation
function create_operation(smf_host, supi, csa) {
...
if(create>0){
    ...
    for(let i=0;i<create;i++) {
        requests[i] = [method, csa + uri, create_body, headers];
    }
...
function process_response_headers(responses) {
   ...
   for (let i=0;i<resp_length;i++) {
      ...
      if (location != null && (location.includes("occ") || location.includes("p1") || location.includes("p2") || location.includes("p3"))) {
...
function update_operation(ids, smf_host, supi, csa) {
...
if(update>0){
    ...
    for(let i=0;i<update;i++) {
        ...
        requests[i] = [method, csa + uri + ids[counter] + "/update", update_body, headers];
        counter++;
    }
...
function release_operation(ids, smf_host, supi, csa) {
...
if(release>0){
    ...
    for(let i=0;i<release;i++) {
        requests[i] = [method, csa + uri + ids[i] + "/release", {}, headers];
    }
