...
/** Send batch messages of 100 cc_create
 *  then 100 cc_update and then 100 cc_release
 *  requests 
 */
export default function() {
    ...
    var ids = create_operation(smf_host, supi, csa);
    if (ids.length > 0) {
...
function create_operation(smf_host, supi, csa) {
    ...
    var create_body = get_create_body(smf_host);
...
function update_operation(ids, smf_host, supi, csa) {
    ...
    var update_body = get_update_body(smf_host);

    for(var i=0;i<ids.length;i++) {
        requests[i] = [method, csa + uri + ids[i] + "/update", update_body, headers];
    }
...
function release_operation(ids, smf_host, supi, csa) {
    ...
    for(var i=0;i<ids.length;i++) {
        requests[i] = [method, csa + uri + ids[i] + "/release"];
    }
