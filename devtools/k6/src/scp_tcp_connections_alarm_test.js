...
export default function() {
		...
		const scp = `http://${__ENV.SCP_HOST}:${__ENV.SCP_PORT}/`;
		
		send_sr_n8_traffic(scp, headers_sr1, headers_sr2, uri_n8);
//		send_pr_n8_traffic(scp, headers_pr1, headers_pr2, uri_n8);
//		send_rr_n8_traffic(scp, headers_rr, uri_n8, `${__ENV.RR}`);
//                send_ms_add_n8_traffic(scp, headers_ms_add, uri_n8);
//                send_ms_modify_n8_traffic(scp, headers_ms_modify, uri_n8);
//                send_ms_remove_n8_traffic(scp, headers_ms_remove, uri_n8);

}

//Function to send traffic for strict routing
function send_sr_n8_traffic(scp, headers_sr1,headers_sr2, uri_n8) {
...
let sr = `${__ENV.SR}`;
    ...
    if(sr>0){
		...
		if( (i % 2) == 0 ){
			requests[i] = [method, scp + uri_n8, body, headers_sr1];
		}

                if( (i % 2) == 1 ){
                        requests[i] = [method, scp + uri_n8, body, headers_sr2];
                }
...
//function send_pr_n8_traffic(scp, headers_pr1, headers_pr2, uri_n8) {
//
//let pr = `${__ENV.PR}`;
//
//    if(pr>0){
...
//
//                if( (i % 2) == 0 ){
//                        requests[i] = [method, scp + uri_n8, body, headers_pr1];
//                }
//
//                if( (i % 2) == 1 ){
//                        requests[i] = [method, scp + uri_n8, body, headers_pr2];
//                }
//
//        }
//    
...
//function send_rr_n8_traffic(scp, headers_rr, uri_n8) {
//
//let rr = `${__ENV.RR}`;
//
//    if(rr>0){
...
//        //create the batch for traffic
//        for(let i=0;i<rr;i++) {
//            requests[i] = [method, scp + uri_n8, body, headers_rr];
//        }
//    
//        let responses = http.batch(requests);
//    
...
//}
//
////Function to send traffic for message screening with adding headers
//function send_ms_add_n8_traffic(scp, headers_ms_add, uri_n8) {
//
//let ms_add = `${__ENV.MS_ADD}`;
//
//    if(ms_add>0){
//
//                const method = "GET";
...
//
//        //create the batch for traffic
//        for(let i=0;i<ms_add;i++) {
//            requests[i] = [method, scp + uri_n8, body, headers_ms_add];
//        }
//
//        let responses = http.batch(requests);
//
...
//        {
//            check(responses[i], {
//                "MS_ADD/N8 traffic: protocol is HTTP/2": (r) => r.proto === 'HTTP/2.0',
//                "MS_ADD/N8 traffic: status was 200": (r) => r.status == 200
//            });
//        }
//
//    }
...
//}
//
////Function to send traffic for message screening with modifying headers
//function send_ms_modify_n8_traffic(scp, headers_ms_modify, uri_n8) {
//
//let ms_modify = `${__ENV.MS_MODIFY}`;
//
//    if(ms_modify>0){
//
//                const method = "GET";
...
//
//        //create the batch for traffic
//        for(let i=0;i<ms_modify;i++) {
//            requests[i] = [method, scp + uri_n8, body, headers_ms_modify];
//        }
//
//        let responses = http.batch(requests);
//
...
//        {
//            check(responses[i], {
//                "MS_MODIFY/N8 traffic: protocol is HTTP/2": (r) => r.proto === 'HTTP/2.0',
//                "MS_MODIFY/N8 traffic: status was 200": (r) => r.status == 200
//            });
//        }
//
//    }
...
//}
//
////Function to send traffic for message screening with removing headers
//function send_ms_remove_n8_traffic(scp, headers_ms_remove, uri_n8) {
//
//let ms_remove = `${__ENV.MS_REMOVE}`;
//
//    if(ms_remove>0){
//
//                const method = "GET";
...
//
//        //create the batch for traffic
//        for(let i=0;i<ms_remove;i++) {
//            requests[i] = [method, scp + uri_n8, body, headers_ms_remove];
//        }
//
//        let responses = http.batch(requests);
//
...
//        {
//            check(responses[i], {
//                "MS_REMOVE/N8 traffic: protocol is HTTP/2": (r) => r.proto === 'HTTP/2.0',
//                "MS_REMOVE/N8 traffic: status was 200": (r) => r.status == 200
//            });
//        }
//
//    }
