...
public class NnrfDiscSearchResultDb
{
    @JsonInclude(JsonInclude.Include.NON_NULL)
    @JsonPropertyOrder({ "valid", "data" })
    public static class Item
    {
        ...
        private static void splitRanges(final SearchResult searchResult)
        {
            searchResult.getNfInstances().forEach(p ->
            {
//                log.info("before={}", p);
...
//                log.info("after={}", p);
        ...
        private static List<SupiRange> splitRangesSupi(final List<SupiRange> ranges)
        {
            return ranges.stream()
                         ...
                         .map(range -> range.split(0)
                                            .stream()
                                            .map(r -> new SupiRange().start(r.start).end(r.end).pattern(r.pattern))
                                            .collect(Collectors.toList()))
                         .reduce(new ArrayList<>(),
                                 (result,
                                  element) ->
        ...
        public Item(final String queryParamNames,
                    final SearchResult data)
        {
            splitRanges(data); // Adapt ranges (SUPI, GPSI) to Trie requirements.
        ...
        public boolean equals(Object other)
        {
            ...
            Item rhs = ((Item) other);
            return (this.queryParamNames == rhs.queryParamNames || this.queryParamNames != null && this.queryParamNames.equals(rhs.queryParamNames))
        ...
        {
            if (!this.queryParamNames.equals(query.getParamNames()))
                return false;
...
//            log.debug("targetNfType={}, gpsi={}, supi={}", targetNfType, gpsi, supi);
    ...
    private static class Db
    {
        ...
        private final Trie<Item> byGpsiRange = new Trie<>('*');
        private final Trie<Item> bySupiRange = new Trie<>('*');
        ...
        public Trie<Item> getByGpsiRange()
        {
        ...
        public Trie<Item> getBySupiRange()
        {
                               ...
                               final Item item)
        {
            ...
            if (range.getPattern() != null)
                ...
                this.byGpsiRange.add(trieKey, item);
        }
                               ...
                               final Item item)
        {
            ...
            if (range.getPattern() != null)
                ...
                this.bySupiRange.add(trieKey, item);
        }
                                  ...
                                  final Item item)
        {
            ...
            if (range.getPattern() != null)
                ...
                this.byGpsiRange.remove(trieKey, item);
        }
                                  ...
                                  final Item item)
        {
            ...
            if (range.getPattern() != null)
                ...
                this.bySupiRange.remove(trieKey, item);
        }
    }

    private static final Logger log = LoggerFactory.getLogger(NnrfDiscSearchResultDb.class);
    ...
    public Set<Item> get(final Query query)
    {
...
//        log.debug("targetNfType={}, gpsi={}, supi={}", targetNfType, gpsi, supi);
        ...
        final Predicate<Item> filter = v -> v.matches(query);
                               ...
                               final Predicate<Item> filter)
    {
        ...
        if (result.isEmpty())
            result = nfProfiles.getByGpsiRange().get(gpsi.substring(gpsi.indexOf('-') + 1), Trie.Strategy.FULL_MATCH, filter);
                               ...
                               final Predicate<Item> filter)
    {
        ...
        if (result.isEmpty())
            result = nfProfiles.getBySupiRange().get(supi.substring(supi.indexOf('-') + 1), Trie.Strategy.FULL_MATCH, filter);
