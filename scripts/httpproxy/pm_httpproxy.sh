...
create_pm_httpproxy(){
	...
	cat scripts/httpproxy/pm-httpproxy.yaml > scripts/httpproxy/temp_pm_httpproxy.yaml
	sed -i 's/namespace_template/'${KUBE_NAMESPACE}'/g' scripts/httpproxy/temp_pm_httpproxy.yaml
	sed -i 's/hostname_template/'${KUBE_HOST}'/g' scripts/httpproxy/temp_pm_httpproxy.yaml
	sed -i 's/ingress_class_template/'${ingress_class}'/g' scripts/httpproxy/temp_pm_httpproxy.yaml
	kubectl --kubeconfig $KUBE_CONFIG  --namespace $KUBE_NAMESPACE create -f scripts/httpproxy/temp_pm_httpproxy.yaml
	rm -rf scripts/httpproxy/temp_pm_httpproxy.yaml
}

delete_pm_httpproxy(){
	kubectl --kubeconfig $KUBE_CONFIG --namespace $KUBE_NAMESPACE delete httpproxy eric-pm-server --ignore-not-found
}
