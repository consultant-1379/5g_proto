#module load python/3.7-addons-requests
...
parser.add_argument('-t', '--traffic', type=str, help="show latency and tps statistics about envoy traffic. possible values are sepp and scp")
...
if args.traffic:
    if not (args.traffic == "sepp" or args.traffic == "scp"):
...
def get_cpu_max_metrics() :
    ...
    responseContainersCpu = requests.get(PROMETHEUS + '/api/v1/query_range', params={'query': "rate(container_cpu_usage_seconds_total{namespace='" + args.statsNamespace + "'}[1m])", 'start':startTime, 'end':endTime, 'step':'5s'})
...
def get_memory_max_metrics() :
    ...
    responseContainersMemory = requests.get(PROMETHEUS + '/api/v1/query_range', params={'query': "container_memory_working_set_bytes{namespace='" + args.statsNamespace + "'}", 'start':startTime, 'end':endTime, 'step':'5s'})
...
def get_tps_avg_metrics() :
    ...
    if trafficIsSepp: #sepp traffic
        queryFilters = "envoy_response_code_class='2',nf='sepp',group='ingress',envoy_listener_address='',roaming_partner=''"
    else: #scp traffic
        queryFilters = "envoy_response_code_class='2',nf='scp',group='ingress',envoy_listener_address=''"
