...
/**
 * Performs the channel configuration and connection according to the client
 * options and the protocol version.
 *
...
public class HttpChannelConnector
{
    ...
    private void connect(EventLoopContext context,
                         Promise<NetSocket> promise)
    {
        netClient.connectInternal(proxyOptions,
                                  server,
                                  peerAddress,
                                  this.options.isForceSni() ? peerAddress.host() : null,
                                  ssl,
                                  useAlpn,
                                  false,
                                  promise,
    ...
    {
        // ch.pipeline().addLast("pcaplogger", new
        // LoggingHandler(LogLevel.INFO,ByteBufFormat.HEX_DUMP)); // echfa
        VertxSocketTracer tracer = null;//VertxSocketTracer.getGlobalTracer();
        if(tracer != null)
        {
    ...
    public Future<HttpClientConnection> wrap(EventLoopContext context,
                                             NetSocket so_)
    {
    ...
    public void httpConnect(EventLoopContext context,
    ...
    private void applyHttp1xConnectionOptions(ChannelPipeline pipeline)
    {
        ...
        pipeline.addLast("codec",
                         new HttpClientCodec(options.getMaxInitialLineLength(),
                                 ...
                                 Promise<HttpClientConnection> future)
    {
        ...
        VertxHandler<Http1xClientConnection> clientHandler = VertxHandler.create(chctx ->
        {
            ...
            Http1xClientConnection conn = new Http1xClientConnection(upgrade ? HttpVersion.HTTP_1_1
                                                                             : version,
                                                                     client,
                                                                     chctx,
                                                                     ssl,
                                ...
                                Promise<HttpClientConnection> future)
    {
        try
        {
            VertxHttp2ConnectionHandler<Http2ClientConnection> clientHandler = Http2ClientConnection.createHttp2ConnectionHandler(client,
                                                                                                                                  metrics,
                                                                                                                                  context,
                                                                                                                                  false,
                                                                                                                                  metric,
    ...
    private void connectFailed(Channel ch,
                               Throwable t,
                               Promise<HttpClientConnection> future)
    {
        if (ch != null)
