...
import io.netty.handler.pcap.MyPcapWriteHandler;
...
/**
 * A channel initializer that will takes care of configuring a blank channel for
 * HTTP to Vert.x {@link io.vertx.core.http.HttpServerRequest}.
 *
...
public class HttpServerWorker implements Handler<Channel>
{
    ...
    private void configurePipeline(Channel ch)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                // Handler that detects whether the HTTP/2 connection preface or just process
                // the request
                // with the HTTP 1.x pipeline to support H2C with prior knowledge, i.e a client
                // that connects
                // and uses HTTP/2 in clear text directly without an HTTP upgrade.
                pipeline.addLast(new Http1xOrH2CHandler()
                {
                    @Override
                    protected void configure(ChannelHandlerContext ctx,
                                             boolean h2c)
                    {
                        if (idle != null)
                        {
    ...
    private void addPcapHandler(Channel ch)
    {
        ...
        if(tracer != null)
        {
        //ch.pipeline().addLast("pcapLogging", new LoggingHandler(LogLevel.INFO, ByteBufFormat.HEX_DUMP)); // echfari
        VertxSocketTracer.getGlobalTracer().addTraceHandler(ch);
        }

        /*
         * try { final var pcap = Files.createTempFile(this.getClass().getName(),
         * ".pcap"); final var fos = new FileOutputStream(pcap.toFile()); final var pwh
         * = PcapWriteHandler.builder().build(fos); ch.pipeline().addLast("pcap",pwh); }
    ...
    private void handleHttp2(Channel ch)
    {
        ...
        /*
        for (var p : ch.pipeline())
        {
            System.out.println("PIPELINE----------------" + p.getKey() + " : " + p.getValue());
        }
        for (var p : ch.parent().pipeline())
        {
            System.out.println("PARENT PIPELINE----------------" + p.getKey() + " : " + p.getValue());
        }
        */
    ...
    VertxHttp2ConnectionHandler<Http2ServerConnection> buildHttp2ConnectionHandler(EventLoopContext ctx,
                                                                                   Handler<HttpServerConnection> handler_)
    {
        HttpServerMetrics metrics = (HttpServerMetrics) server.getMetrics();
        VertxHttp2ConnectionHandler<Http2ServerConnection> handler = new VertxHttp2ConnectionHandlerBuilder<Http2ServerConnection>().server(true)
                                                                                                                                    ...
                                                                                                                                    {
                                                                                                                                        Http2ServerConnection conn = new Http2ServerConnection(ctx,
                                                                                                                                        ...
                                                                                                                                        if (metrics != null)
                                                                                                                                        {
                                                                                                                                            conn.metric(metrics.connected(conn.remoteAddress(),
                                                                                                                                                                          conn.remoteName()));
                                                                                                                                        }
                                                                                                                                        return conn;
                                                                                                                                    })
    ...
    private void configureHttp1OrH2C(ChannelPipeline pipeline)
    {
        ...
        {
            // only add ChunkedWriteHandler when SSL is enabled otherwise it is not needed
            // as FileRegion is used.
    ...
    void configureHttp1(ChannelPipeline pipeline)
    {
        ...
        {
            Http1xServerConnection conn = new Http1xServerConnection(streamContextSupplier, sslHelper, options, chctx, context, serverOrigin, metrics);
            return conn;
        });
