package Playlist::002_Upgrade_SC;
...
sub main {
    ...
    $rc = General::Playlist_Operations::execute_step( \&Initialize_Job_Environment_P002S01, \&Fallback001_P002S99 );
    return $rc if $rc < 0;

    $rc = General::Playlist_Operations::execute_step( \&Check_Job_Parameters_P002S02, \&Fallback001_P002S99 );
    return $rc if $rc < 0;

    $rc = General::Playlist_Operations::execute_step( \&Check_Configuration_Files_If_Specified_P002S03, \&Fallback001_P002S99 );
    return $rc if $rc < 0;

    $rc = General::Playlist_Operations::execute_step( \&KPI_Entry_Collection_P002S04, \&Fallback001_P002S99 );
    return $rc if $rc < 0;

    $rc = General::Playlist_Operations::execute_step( \&Global_Preprocessing_P002S05, \&Fallback001_P002S99 );
    return $rc if $rc < 0;

    $rc = General::Playlist_Operations::execute_step( \&Collect_Logs_Before_Upgrade_If_Wanted_P002S06, \&Fallback001_P002S99 );
    return $rc if $rc < 0;

    $rc = General::Playlist_Operations::execute_step( \&Upgrade_Preprocessing_P002S07, \&Fallback001_P002S99 );
    return $rc if $rc < 0;

    $rc = General::Playlist_Operations::execute_step( \&Upgrade_P002S08, \&Fallback001_P002S99 );
    return $rc if $rc < 0;

    $rc = General::Playlist_Operations::execute_step( \&Upgrade_Postprocessing_P002S09, \&Fallback001_P002S99 );
    return $rc if $rc < 0;

    $rc = General::Playlist_Operations::execute_step( \&Global_Postprocessing_P002S10, \&Fallback001_P002S99 );
    return $rc if $rc < 0;

    $rc = General::Playlist_Operations::execute_step( \&Collect_Logs_If_Wanted_P002S11, \&Fallback001_P002S99 );
    return $rc if $rc < 0;

    $rc = General::Playlist_Operations::execute_step( \&KPI_Upgrade_And_Exit_Collection_P002S12, \&Fallback001_P002S99 );
    return $rc if $rc < 0;

    $rc = General::Playlist_Operations::execute_step( \&Cleanup_Job_Environment_P002S13, \&Fallback001_P002S99 );
    return $rc if $rc < 0;
...
#
# -----------------------------------------------------------------------------
sub Initialize_Job_Environment_P002S01 {
...
#
# -----------------------------------------------------------------------------
sub Check_Job_Parameters_P002S02 {

    my $rc;

    $rc = General::Playlist_Operations::execute_task( { "task-ref" => \&Check_Job_Parameters_P002S02T01, "dry-run-job" => $::JOB_PARAMS{'DRY_RUN'}, "dry-run-task" => 1 } );
    return $rc if $rc < 0;
    ...
    #
    # -----------------------------------------------------------------------------
    sub Check_Job_Parameters_P002S02T01 {
...
#
# -----------------------------------------------------------------------------
sub Check_Configuration_Files_If_Specified_P002S03 {
    ...
    if ($::JOB_PARAMS{'SKIP_DEPLOY_UPGRADE_SC'} eq "no" && $::JOB_PARAMS{'USING_EVNFM'} eq "no" && $::JOB_PARAMS{'SKIP_RESOURCE_CHECK'} eq "no") {
        ...
        if ($use_legacy_deploy_upgrade == 0) {
            ...
            # Mark that a check if needed resources for the upgrade shall be checked against available resources.
...
#
# -----------------------------------------------------------------------------
sub Global_Preprocessing_P002S05 {
...
#
# -----------------------------------------------------------------------------
sub KPI_Entry_Collection_P002S04 {
    ...
    if ($::JOB_PARAMS{'SKIP_KPI_COLLECTION'} eq "no" && $::JOB_PARAMS{'COLLECT_ENTRY_KPI_VALUES'} eq "yes") {
        ...
        if ($::JOB_PARAMS{'SKIP_TRAFFIC_DELAY'} eq "no") {
            $rc = General::Playlist_Operations::execute_task( { "task-ref" => \&Wait_For_Entry_Time_To_Expire_P002S04T01 } );
            return $rc if $rc < 0;
        }
    ...
    $rc = General::Playlist_Operations::execute_task( { "task-ref" => \&Record_Upgrade_Start_Time_P002S04T02 } );
    return $rc if $rc < 0;
    ...
    #
    # -----------------------------------------------------------------------------
    sub Wait_For_Entry_Time_To_Expire_P002S04T01 {
    ...
    #
    # -----------------------------------------------------------------------------
    sub Record_Upgrade_Start_Time_P002S04T02 {
...
#
# -----------------------------------------------------------------------------
sub Collect_Logs_Before_Upgrade_If_Wanted_P002S06 {
...
#
# -----------------------------------------------------------------------------
sub Upgrade_Preprocessing_P002S07 {

    my $rc;

    $rc = General::Playlist_Operations::execute_task( { "task-ref" => \&Playlist::907_Upgrade_Preprocess::main, "dry-run-job" => $::JOB_PARAMS{'DRY_RUN'} } );
    return $rc if $rc < 0;
...
sub Upgrade_P002S08 {
    ...
    $rc = General::Playlist_Operations::execute_task( { "task-ref" => \&Show_Tasks_To_Be_Done_P002S08T01 } );
    return $rc if $rc < 0;

    $rc = General::Playlist_Operations::execute_task( { "task-ref" => \&Playlist::912_Upgrade::main, "dry-run-job" => $::JOB_PARAMS{'DRY_RUN'} } );
    return $rc if $rc < 0;
    ...
    #
    # -----------------------------------------------------------------------------
    sub Show_Tasks_To_Be_Done_P002S08T01 {
...
#
# -----------------------------------------------------------------------------
sub Upgrade_Postprocessing_P002S09 {

    my $rc;

    $rc = General::Playlist_Operations::execute_task( { "task-ref" => \&Playlist::908_Upgrade_Postprocess::main, "dry-run-job" => $::JOB_PARAMS{'DRY_RUN'} } );
    return $rc if $rc < 0;
...
#
# -----------------------------------------------------------------------------
sub Global_Postprocessing_P002S10 {
...
#
# -----------------------------------------------------------------------------
sub Collect_Logs_If_Wanted_P002S11 {
...
#
# -----------------------------------------------------------------------------
sub KPI_Upgrade_And_Exit_Collection_P002S12 {
    ...
    if ($::JOB_PARAMS{'SKIP_KPI_COLLECTION'} eq "no" && $::JOB_PARAMS{'COLLECT_EXIT_KPI_VALUES'} eq "yes") {
        ...
        if ($::JOB_PARAMS{'SKIP_TRAFFIC_DELAY'} eq "no") {
            $rc = General::Playlist_Operations::execute_task( { "task-ref" => \&Wait_For_Exit_Time_To_Expire_P002S12T01 } );
            return $rc if $rc < 0;
        }
    ...
    #
    # -----------------------------------------------------------------------------
    sub Wait_For_Exit_Time_To_Expire_P002S12T01 {
...
#
# -----------------------------------------------------------------------------
sub Cleanup_Job_Environment_P002S13 {
...
eric-sc-values.yaml file which is used for the upgrade.
...
then the upgraded CNF's will be according to the settings in the network
...
#2345678901234567890123456789012345678901234567890123456789012345678901234567890
            'usage'         => <<EOF,
Specify the timeout value to use when upgrading the CRD software using the helm
command, if not specified then the default value will be used.
...
check done before an Upgrade. If it's estimated that there are not enough
resources for an Upgrade then only a warning message is printed and the Upgrade
will continue anyway.

Select "no" which is also the default value if not specified, to have the
Playlist stop if there is not enogh resources for an Upgrade.
...
If only the SKIP_UPGRADE_CRD parameter is given then SKIP_DEPLOY_UPGRADE_CRD will
...
This parameter controls if the actual upgrade of the CRD Helm chart should be
skipped but everything else should be performed like upgrading SC etc.
If the value is set to "yes" then the playlist will skip the upgrade of the
...
The default if this parameter is not given is to perform the full upgrade of
the CRD software.
EOF
            'validity_mask' => '(yes|no)',
            'value'         => "",
...
CPU and Memory resources before attempting to upgrade the software.
If the value is set to "yes" then the playlist will skip the resource check which
might cause failures during the upgrade if there are not enough resources available.
If the value is set to "no", which is also the default, then a check of available
resources is performed before doing the upgrade and the playlist will stop with
...
If only the SKIP_UPGRADE_SC parameter is given then SKIP_DEPLOY_UPGRADE_SC will
be set to the same value.
...
This parameter controls if the actual upgrade of the SC Helm chart should be
skipped but everything else should be performed like upgrading CRD etc.
If the value is set to "yes" then the playlist will skip the upgrade of the SC
Helm chart and checking of Cluster status.

This is a special job parameter that will be used by EVNFM since they want to
do the actual upgrade of the Helm chart using their own scripts.
...
This Playlist script is used for performing Upgrade of an ESC node and it
