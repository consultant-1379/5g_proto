package Playlist::905_Deploy_Preprocess;
...
sub main {
    ...
    $rc = General::Playlist_Operations::execute_step( \&Show_Message_P905S01, undef );
    return $rc if $rc < 0;

    $rc = General::Playlist_Operations::execute_step( \&Process_Playlist_Extensions_Command_Files_P905S02, undef );
    return $rc if $rc < 0;
...
#
# -----------------------------------------------------------------------------
sub Show_Message_P905S01 {

    my $rc;

    $rc = General::Playlist_Operations::execute_task( { "task-ref" => \&Show_Tasks_To_Be_Done_P905S01T01 } );
    return $rc if $rc < 0;
    ...
    #
    # -----------------------------------------------------------------------------
    sub Show_Tasks_To_Be_Done_P905S01T01 {
        ...
        General::Message_Generation::print_message_box(
            {
                "header-text"   => "Deployment Pre Processing",
                "messages"      => [
                    "",
                    "Tasks to be done:",
...
#
# -----------------------------------------------------------------------------
sub Process_Playlist_Extensions_Command_Files_P905S02 {
    ...
    $::JOB_PARAMS{'PLAYLIST_EXTENSION_PATH'} = "$::JOB_PARAMS{'_PACKAGE_DIR'}/playlist_extensions/$app_type/deploy/pre";
...
This Playlist is used for executing tasks before any Deployment is performed.
