package Playlist::908_Upgrade_Postprocess;
...
sub main {
    ...
    $rc = General::Playlist_Operations::execute_step( \&Show_Message_P908S01, undef );
    return $rc if $rc < 0;

    $rc = General::Playlist_Operations::execute_step( \&Process_Playlist_Extensions_Command_Files_P908S02, undef );
    return $rc if $rc < 0;
...
#
# -----------------------------------------------------------------------------
sub Show_Message_P908S01 {

    my $rc;

    $rc = General::Playlist_Operations::execute_task( { "task-ref" => \&Show_Tasks_To_Be_Done_P908S01T01 } );
    return $rc if $rc < 0;
    ...
    #
    # -----------------------------------------------------------------------------
    sub Show_Tasks_To_Be_Done_P908S01T01 {
        ...
        General::Message_Generation::print_message_box(
            {
                "header-text"   => "Upgrade Post Processing",
                "messages"      => [
                    "",
                    "Tasks to be done:",
...
#
# -----------------------------------------------------------------------------
sub Process_Playlist_Extensions_Command_Files_P908S02 {
    ...
    $::JOB_PARAMS{'PLAYLIST_EXTENSION_PATH'} = "$::JOB_PARAMS{'_PACKAGE_DIR'}/playlist_extensions/$app_type/upgrade/post";
...
This Playlist is used for executing tasks after an Upgrade has been done.
