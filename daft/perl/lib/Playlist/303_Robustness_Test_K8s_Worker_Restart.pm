package Playlist::303_Robustness_Test_K8s_Worker_Restart;
...
sub main {
    ...
    # Set Job Type
    $::JOB_PARAMS{'JOBTYPE'} = "ROBUSTNESS_TEST_K8S_WORKER_RESTART";
    ...
    $rc = General::Playlist_Operations::execute_step( \&Perform_Pre_Test_Checks_P303S01, \&Fallback001_P303S99 );
    return $rc if $rc < 0;

    $rc = General::Playlist_Operations::execute_step( \&Perform_Test_Case_P303S02, \&Fallback001_P303S99 );
    return $rc if $rc < 0;

    $rc = General::Playlist_Operations::execute_step( \&Perform_Post_Test_Checks_P303S03, \&Fallback001_P303S99 );
    return $rc if $rc < 0;
...
#
# -----------------------------------------------------------------------------
sub Perform_Pre_Test_Checks_P303S01 {
...
#
# -----------------------------------------------------------------------------
sub Perform_Test_Case_P303S02 {

    my $length;
    my @ip_addresses = ADP::Kubernetes_Operations::get_node_ip( { "type" => "worker" } );
    my $message;
    ...
    $::JOB_PARAMS{'P303_NODE_CNT'}    = exists $::JOB_PARAMS{'P303_NODE_CNT'}    ? $::JOB_PARAMS{'P303_NODE_CNT'}    : 0;
    $::JOB_PARAMS{'P303_EXECUTE_P303S02T01'} = exists $::JOB_PARAMS{'P303_EXECUTE_P303S02T01'} ? $::JOB_PARAMS{'P303_EXECUTE_P303S02T01'} : 1;
    $::JOB_PARAMS{'P303_EXECUTE_P303S02T02'} = exists $::JOB_PARAMS{'P303_EXECUTE_P303S02T02'} ? $::JOB_PARAMS{'P303_EXECUTE_P303S02T02'} : 1;
    $::JOB_PARAMS{'P303_EXECUTE_P303S02T03'} = exists $::JOB_PARAMS{'P303_EXECUTE_P303S02T03'} ? $::JOB_PARAMS{'P303_EXECUTE_P303S02T03'} : 1;
    $::JOB_PARAMS{'P303_EXECUTE_P303S02T04'} = exists $::JOB_PARAMS{'P303_EXECUTE_P303S02T04'} ? $::JOB_PARAMS{'P303_EXECUTE_P303S02T04'} : 1;
    $::JOB_PARAMS{'P303_SKIPPED_WORKERS'}    = exists $::JOB_PARAMS{'P303_SKIPPED_WORKERS'}    ? $::JOB_PARAMS{'P303_SKIPPED_WORKERS'}    : 0;

    for (; $::JOB_PARAMS{'P303_NODE_CNT'} <= $#ip_addresses;) {
        $::JOB_PARAMS{'P303_NODE_IP'} = $ip_addresses[$::JOB_PARAMS{'P303_NODE_CNT'}];
        $::JOB_PARAMS{'P303_NODE_NAME'} = $node_names[$::JOB_PARAMS{'P303_NODE_CNT'}];
        ...
        $::JOB_PARAMS{'TEST_DESCRIPTION'} = sprintf "Robustness Test Case %d Node Reboot (%s)", $::JOB_PARAMS{'P303_NODE_CNT'}+1, $::JOB_PARAMS{'P303_NODE_NAME'};

        # Print a progress message
        $message = sprintf "# Restart %d of %d, Node IP=%s #", $::JOB_PARAMS{'P303_NODE_CNT'} + 1, $#ip_addresses + 1, $::JOB_PARAMS{'P303_NODE_IP'};
        ...
        if (exists $tool_nodes{$::JOB_PARAMS{'P303_NODE_NAME'}}) {
            # This node is used for tools and should be excluded from the test.
            # Step the counter and take next worker
            $::JOB_PARAMS{'P303_NODE_CNT'}++;
            General::Logging::log_user_message("This node is hosting tools and will be skipped from the test");
            push @::JOB_STATUS, "(-) $::JOB_PARAMS{'TEST_DESCRIPTION'} skipped because node is hosting tools";
            next;
        }

        if ($::JOB_PARAMS{'P303_EXECUTE_P303S02T01'} == 1) {
            $::JOB_PARAMS{'P303_WORKER_SKIPPED'} = 0;
            $rc = General::Playlist_Operations::execute_task( { "task-ref" => \&Check_Pods_In_Node_Before_Restart_P303S02T01 } );
            return $rc if $rc < 0;
            $::JOB_PARAMS{'P303_EXECUTE_P303S02T01'} = 0;
            if ($::JOB_PARAMS{'P303_WORKER_SKIPPED'} == 1) {
                $::JOB_PARAMS{'P303_SKIPPED_WORKERS'}++;
                # Step the counter and take next worker
                $::JOB_PARAMS{'P303_NODE_CNT'}++;
                next;
            }
        ...
        if ($::JOB_PARAMS{'P303_EXECUTE_P303S02T02'} == 1) {
            $rc = General::Playlist_Operations::execute_task( { "task-ref" => \&Restart_Node_P303S02T02 } );
            return $rc if $rc < 0;
            $::JOB_PARAMS{'P303_EXECUTE_P303S02T02'} = 0;
        }

        if ($::JOB_PARAMS{'P303_EXECUTE_P303S02T03'} == 1) {
            $rc = General::Playlist_Operations::execute_task( { "task-ref" => \&Wait_For_Node_To_Become_Reachable_P303S02T03 } );
            return $rc if $rc < 0;
            $::JOB_PARAMS{'P303_EXECUTE_P303S02T03'} = 0;
        }
        ...
        if ($::JOB_PARAMS{'P303_EXECUTE_P303S02T04'} == 1) {
            $rc = General::Playlist_Operations::execute_task( { "task-ref" => \&Check_Pods_In_Node_After_Restart_P303S02T04 } );
            return $rc if $rc < 0;
            $::JOB_PARAMS{'P303_EXECUTE_P303S02T04'} = 0;
        }

        # Step the counter and reset the executed tasks
        $::JOB_PARAMS{'P303_NODE_CNT'}++;
        $::JOB_PARAMS{'P303_EXECUTE_P303S02T01'} = 1;
        $::JOB_PARAMS{'P303_EXECUTE_P303S02T02'} = 1;
        $::JOB_PARAMS{'P303_EXECUTE_P303S02T03'} = 1;
        $::JOB_PARAMS{'P303_EXECUTE_P303S02T04'} = 1;
    }
    ...
    delete $::JOB_PARAMS{'P303_EXECUTE_P303S02T01'};
    delete $::JOB_PARAMS{'P303_EXECUTE_P303S02T02'};
    delete $::JOB_PARAMS{'P303_EXECUTE_P303S02T03'};
    delete $::JOB_PARAMS{'P303_EXECUTE_P303S02T04'};
    delete $::JOB_PARAMS{'P303_NODE_CNT'};
    delete $::JOB_PARAMS{'P303_NODE_IP'};
    delete $::JOB_PARAMS{'P303_WORKER_SKIPPED'};
    ...
    #
    # -----------------------------------------------------------------------------
    sub Check_Pods_In_Node_Before_Restart_P303S02T01 {

        my $command = sprintf "%s get pods --all-namespaces -o wide --field-selector spec.nodeName=%s", $::JOB_PARAMS{'KUBECTL_EXECUTABLE'}, $::JOB_PARAMS{'P303_NODE_NAME'};
        my @faults = ();
        my $namespace;
        my $node_name = $::JOB_PARAMS{'P303_NODE_NAME'};
        my $podname;
        my $rc = 0;
        my @result;
        my $status;

        General::Logging::log_user_message("Checking for PODS in node: $::JOB_PARAMS{'P303_NODE_NAME'}");
        ...
        if (@faults) {
            ...
            push @::JOB_STATUS, "(x) $::JOB_PARAMS{'TEST_DESCRIPTION'} failed because some pods are not having STATUS=Running before the test";
            $::JOB_PARAMS{'P303_WORKER_SKIPPED'} = 1;
        } else {
    ...
    #
    # -----------------------------------------------------------------------------
    sub Restart_Node_P303S02T02 {

        my $command = sprintf "ssh -o StrictHostKeyChecking=no -o BatchMode=Yes -o loglevel=ERROR eccd\@%s 'sudo -n reboot'", $::JOB_PARAMS{'P303_NODE_IP'};
        my $rc = 0;

        General::Logging::log_user_message("Restarting node on IP address: $::JOB_PARAMS{'P303_NODE_IP'}");
        ...
        if ($rc == 0) {
            ...
            $command = sprintf "ping -c 1 -W 1 %s", $::JOB_PARAMS{'P303_NODE_IP'};
            while ($stop_time > time()) {
                ...
                if ($rc == 0) {
                    ...
                    General::Logging::log_user_message("Connection lost for node on IP address $::JOB_PARAMS{'P303_NODE_IP'}, restart in progress");
    ...
    #
    # -----------------------------------------------------------------------------
    sub Wait_For_Node_To_Become_Reachable_P303S02T03 {

        my $command = sprintf "ping -c 1 -W 5 %s", $::JOB_PARAMS{'P303_NODE_IP'};
        my $node_reachable = 0;
        my $stop_time;
        my $rc = 0;

        General::Logging::log_user_message("Waiting a maximum of $::JOB_PARAMS{'TIMEOUT_NODE_REACHABLE'} seconds for node IP $::JOB_PARAMS{'P303_NODE_IP'} to be pingable");
        ...
        if ($stop_time > time()) {
            ...
            General::Logging::log_user_message(sprintf "Waiting a maximum of %d seconds for node IP %s to be reachable by SSH", $stop_time - time(), $::JOB_PARAMS{'P303_NODE_IP'});
            $command = sprintf "ssh-keyscan -T 5 %s", $::JOB_PARAMS{'P303_NODE_IP'};
            while ($stop_time > time()) {
                $rc = General::OS_Operations::send_command(
                    {
                        "command"       => "$command",
        ...
        if ($rc == 0 && $node_reachable == 1) {
            General::Logging::log_user_message("Node on IP address $::JOB_PARAMS{'P303_NODE_IP'} is reachable");
            return 0;
        } else {
            General::Logging::log_user_error_message("Node on IP address $::JOB_PARAMS{'P303_NODE_IP'} is not reachable");
            push @::JOB_STATUS, "(x) $::JOB_PARAMS{'TEST_DESCRIPTION'} failed because node on IP address $::JOB_PARAMS{'P303_NODE_IP'} is not reachable after restart";
            return 1;
        }
    }
    ...
    #
    # -----------------------------------------------------------------------------
    sub Check_Pods_In_Node_After_Restart_P303S02T04 {

        my $command = sprintf "%s get pods --all-namespaces -o wide --field-selector spec.nodeName=%s", $::JOB_PARAMS{'KUBECTL_EXECUTABLE'}, $::JOB_PARAMS{'P303_NODE_NAME'};
        my @faults = ();
        my $namespace;
        my $node_name = $::JOB_PARAMS{'P303_NODE_NAME'};
        my $perform_status_check = 1;   # Change this value to 0 to have the old behavior where no check was done
        my $podname;
        my $rc = 0;
        my @result;
        my $status;
        ...
        if (@faults) {
            General::Logging::log_user_error_message("The following faults was detected after the test and for this reason the test is marked as failed:\n  " . join "\n  ", @faults);
            push @::JOB_STATUS, "(x) $::JOB_PARAMS{'TEST_DESCRIPTION'} failed because some pods are not having STATUS=Running after the test";
        } else {
...
#
# -----------------------------------------------------------------------------
sub Perform_Post_Test_Checks_P303S03 {
    ...
    if (exists $::JOB_PARAMS{'P303_SKIPPED_WORKERS'} && $::JOB_PARAMS{'P303_SKIPPED_WORKERS'} > 0) {
        # One or more workers has been skipped, mark job as failed
        General::Logging::log_user_error_message("Test case failed because $::JOB_PARAMS{'P303_SKIPPED_WORKERS'} were skipped");
...
This Playlist performs a robustness test case that verifies worker node restarts.
